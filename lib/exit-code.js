...
function exitCodeAssertions (chai) {
  const { assert, Assertion, expect } = chai;

  assert.exitCode = (cli, expected, message) => {
    if (Array.isArray(expected)) {
      oneOf.call(new Assertion(cli), expected, message);
    }
    else {
      expect(cli).to.exit.with.code(expected, message);
    }
  };

  assert.notExitCode = (cli, expected, message) => {
    if (Array.isArray(expected)) {
      oneOf.call(new Assertion(cli).not, expected, message);
    }
    else {
      expect(cli).to.not.exit.with.code(expected, message);
    }
  };
  ...
  function exitCodeChainingBehavior () {
    ...
    assert.containsAllKeys(cli, ["exitCode"]);
    assert.isNumber(cli.exitCode);
  ...
  function exitCodeAssertion (expectedExitCode, message) {
    ...
    expect(actualExitCode, message).to.be.a("number");
    expect(expectedExitCode, message).to.be.a("number");
