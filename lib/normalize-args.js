...
const parseCommand = require('string-argv');  // possible alternative: parse-spawn-args
const detectType = require('type-detect');

module.exports = normalizeArgs;

/**
 * This function normalizes the arguments of the `chaiExec.sync()` and `chaiExec.async()`
...
function normalizeArgs (params) {
  ...
  let { command, args, options, callback } = shiftArgs(params);
  let commandArgs = [];

  if (typeof command === 'string') {
    // Convert the `command` argument from a string to an array
...
function validateArgs (command, args, options, callback) {
  const preface = 'chai-exec was called with invalid arguments.';

  if (command === undefined || command === null) {
    throw new Error(
      `${preface} The command to execute is missing.`
    );
  }

  if (typeof command !== 'string') {
    throw new Error(
      `${preface} The command to execute should be a string or an array, not ` +
      friendlyType(command)
    );
  }

  if (!Array.isArray(args)) {
    throw new Error(
      `${preface} The command arguments should be a string or an array, not ` +
      friendlyType(args)
    );
  }
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (typeof arg !== 'string') {
      throw new Error(
        `${preface} The command arguments should be strings, but argument #${i + 1} is ` +
        friendlyType(arg)
      );
    }
  }
  ...
  if (typeof options !== 'object') {
    throw new Error(
      `${preface} The options should be an object, not ` +
  ...
  if (callback !== undefined && callback !== null) {
    if (typeof callback !== 'function') {
      throw new Error(
        `${preface} The callback should be a function, not ` +
        friendlyType(callback)
      );
    }
  }
}
...
/**
 * Splits an argument string (e.g. "git commit -a -m fixed_stuff")
...
function splitArgString (argString) {
  try {
    return parseCommand(argString);
  }
  catch (error) {
    throw new Error(
      `chai-exec could not parse the string: ${argString}\n` +
      error.message
    );
  }
}
